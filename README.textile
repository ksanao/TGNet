...
**Riba-Grognuz, Keller, Falquet, Xenarios & Wurm (submitted) Visualization and quality assessment of _de novo_ genome assemblies of emerging model organisms.**
...
* "Cytoscape":http://www.cytoscape.org/ (tested with version 2.7.0)
...
**Input**  : assembly fasta [1], transcript fatsa [2], AGP scaffolding file [3], blat alignment (instead fasta) [4]
      ...
      *# AGP file specification: http://www.ncbi.nlm.nih.gov/projects/genome/assembly/agp/AGP_Specification.shtml
         requires a set of contiguous sequences separated by gaps in AGP file
      *# blat alignment in psl format (transcripts to genome assembly)
**Output** : tab delimited files with nodes, edges and properties
...
The @example/input@ directory contains some sequence (FASTA) and scaffolding (AGP) data required to generate the equivalent of Figure 1 from *Riba-Grognuz et al*. These input files are subset from the "fire ant genome project (Wurm et al 2011)":http://www.pnas.org/cgi/doi/10.1073/pnas.1009690108 . These files can be processed as follows:
  ...
  *  **On contig level**:
        @./makeCytoscapeNetwork.sh -g example/input/ExampleContig.fasta -t example/input/ExampleTranscript.fasta -a example/input/Example.agp -n Example@
        or
        @./makeCytoscapeNetwork.sh -p example/input/ExampleContig.psl -a example/input/Example.agp -n Example@
...
* **Importing network**
    ...
    **  Tick a check-box Transcfer first line as attribute names
    **  Import
...
* **My node sizes are not scaled according respective sequence size**
